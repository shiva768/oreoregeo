...
│   ├── ProvisionalCheckinScreen.kt / ProvisionalCheckinViewModel.kt  # Background-tracked provisional checkins
...
└── util/
    ...
    └── LocationTrackingWorker.kt     # WorkManager worker; tracks location every 15 min in background
...
#### `provisional_checkins` Table
- Background-tracked location candidates; generated by `LocationTrackingWorker` (every 15 min)
- User reviews and confirms/dismisses them via `ProvisionalCheckinScreen`
...
8. **Google Drive Backup**: No `google-services.json` or Firebase required. Drive API uses `GoogleAccountCredential` + `DriveScopes.DRIVE_FILE`. Requires an OAuth Android client in Google Cloud Console with the app's debug/release SHA-1 fingerprint registered.
...
- **app/build.gradle.kts**: Dependencies, test runners, ktlint/detekt config
...
- **Drive backup fails with `UnregisteredOnApiConsole`**: Register the app's SHA-1 fingerprint in Google Cloud Console under the OAuth Android client
